...
* **OIDC v1 (NF-017):** generic OIDC (Authentik first) — Admin → SSO overrides `OIDC_*` env at request time; see [`OIDC_IDP.md`](../product/OIDC_IDP.md). Invite/link by default; optional JIT via Admin toggle or `OIDC_JIT_PROVISIONING`. Same `kh_session` cookie as password login.
