...
1. **First-class tables** under `projects`: `project_milestones`, `project_tasks`, `project_task_raci`.
...
4. **RACI parties are workspace users**; exactly one Accountable (`A`) per task when set.
