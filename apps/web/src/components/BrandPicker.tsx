...
import { clearBrandAction, setBrandAction } from '../lib/brand-actions';
    ...
    <section id="brand" className="kh-ops-panel scroll-mt-20">
      ...
      <div className="kh-ops-card-body">
        <p className="mt-0 mb-4 text-sm text-ink-muted">
          {locked ? t('brandLockedBlurb') : t('brandBlurb')}
        </p>
          ...
          <div className="mt-4">
            <Button
              ...
              onClick={() => {
                ...
                startTransition(async () => {
                  await clearBrandAction();
                  pushToast(t('brandResetToDefault'));
                  router.refresh();
                });
              }}
            >
              {t('brandUseDefault', { brand: brandMeta[defaultBrand].label })}
            </Button>
          </div>
        ) : null}
      </div>
