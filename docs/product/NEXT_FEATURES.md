...
| **D — Object storage** | **NF-006** BlobStore + **s3** + Admin → Storage **done**. Avatars still local (Ops-2). **NF-007** Azure on same Storage page **with Entra IdP (NF-012)** | OneDrive/SharePoint |
...
| NF-006 | **BlobStore + S3-compatible storage** — shared object port for backups, avatars, future imports/exports | `partial` — port + `s3` + offsite dumps; avatars still local | Wire avatars/imports (Ops-2); Azure as NF-007 | Wave **D**. Package `@project-knowledge-hub/blob-store`. |
