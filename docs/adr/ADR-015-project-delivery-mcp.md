...
1. **First-class tables** under `projects`: `project_milestones`, `project_tasks`, `project_task_raci`, and hybrid `project_stakeholders` (durable roster + optional `reports_to_user_id`; list merges owner + RACI-derived people).
...
4. **RACI / stakeholder parties are workspace users**; exactly one Accountable (`A`) per task when set; reports-to cycles are rejected.
5. **Knowledge lifecycle unchanged** — agents still only draft knowledge; delivery and documentation remain separate ledgers linked by `projectId`.

## Consequences

* Project page and MCP can treat delivery as structured data.
