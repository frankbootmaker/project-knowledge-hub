...
| NF-017 | **OIDC sign-in (Authentik first)** — generic OIDC alongside local passwords; invite/link by default, optional JIT | `done` (v1) — env bootstrap + Admin → SSO override, login button, PKCE callback, optional JIT | Group→role mapping; multiple issuers in one UI | Brief: [`OIDC_IDP.md`](OIDC_IDP.md). Operator guide: [`OIDC_AUTHENTIK_INTEGRATION_GUIDE.md`](OIDC_AUTHENTIK_INTEGRATION_GUIDE.md). Wave **F** / IdP. NF-012 Entra reuses this path. |
