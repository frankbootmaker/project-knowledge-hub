...
| **E — Ops polish** | **NF-009** support dump + stale chip + worker email/webhook alerts; **NF-014** external status REST/MCP (`monitoring:read`) | Log shipping |
...
| NF-009 | **Ops observability** — log retention/export, lightweight alerting, redacted support dump | `partial` — support dump + stale chip + worker email/webhook (`BACKUP_STALE_ALERT_INTERVAL_MS`, `ALERT_WEBHOOK_URL`) | Log export / richer alert routing | Wave **E**. [`OPERATIONS.md`](../deployment/OPERATIONS.md) Ops-4. |
