...
| NF-018 | **Project Delivery** — milestones, epic→story→task, dates, RACI + handoffs, RAID, baseline/change register, delivery Timeline fishbone; REST/UI + MCP `pm:read`/`pm:write` | `partial` — local MVP on `feature/project-delivery` | Local smoke; then merge when stable (not Dokploy until ready) | Briefs: [`PROJECT_DELIVERY.md`](PROJECT_DELIVERY.md), [`PROJECT_RAID.md`](PROJECT_RAID.md), [`PROJECT_BASELINE.md`](PROJECT_BASELINE.md). ADR-015/016/017. Wave **F**. |
