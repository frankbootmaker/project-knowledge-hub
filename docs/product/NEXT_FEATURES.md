...
| NF-018 | **Project Delivery** — milestones, tasks, dates, RACI, hybrid stakeholders (roster + org chart) under Projects; REST/UI + MCP `pm:read`/`pm:write` | `partial` — local MVP on `feature/project-delivery` | Local smoke; then merge when stable (not Dokploy until ready) | Brief: [`PROJECT_DELIVERY.md`](PROJECT_DELIVERY.md). ADR-015. Wave **F**. |
